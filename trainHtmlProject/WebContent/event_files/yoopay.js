...
var FundCollectionPrice = {
    ...
    //Add or del price option which discount related and hidden price which referral related
...
var BackgroupOperation = {
    ...
    changeUserServiceStatus:function(){
        ...
        var r = confirm("你的操作：修改用户费率\n修改内容：开始时间="+startDate+" 结束时间="+endDate+" 服务版本="+type+" 是否开通信用卡="+creditCard +" 是否开通自定义票="+custonTicket+" 是否开启签到系统="+checkin);
        if(creditCard == 'open'){
            creditCard = true;
...
var Collect={
    ...
    validateEventCollectSend:function(){
        var fromName=$.trim($('#fromName').val());
        var replyToEmail=$.trim($('#replyToEmail').val());
        var emails=$.trim($('#send_email').val());
        var subject=$.trim($('#subject').val());
...
var GatewayPayment = {
    ...
    showGatewayUSDTip : function(srcPage,USD2CNYRate){
        ...
        else if(srcPage == "payment" || srcPage == "add_fund" || srcPage == "yp_service"){
...
var FundPayment = {
    ...
    //检查支付人的基本信息
    _checkPayerBasicForm : function(paymentWMsgDiv, payerInfoRequired, payerWorkInfoRequired, isLogin){
    ...
    checkStepTwoForm : function(type,payerInfoRequired,payerWorkInfoRequired,isLogin,questionSize){
        var paymentForm = $("#payment_form");
...
var FundTransfer = {
    checkLinkTransferForm : function(minAmount) {
        ...
        var gatewayCheckFlag = GatewayPayment.checkGatewayForm($("#linktransfer_wrong_msg_div"));
        if(!gatewayCheckFlag) {
            return false;
        }
        var paymentForm = $("#payment_form");
...
var YoopayLink={
    initHeaderTips : function() {
        if (document.getElementById('header_yoopay_link') != null){
            $("#header_yoopay_link").bt(
            {
                trigger: ['mouseover', 'mousedown'],
                contentSelector: "$('#header_yoopay_link_contents').html()",
                fill: '#E9F4E3',
                cssStyles: {
                    color: 'black', 
                    fontSize: '14px'
